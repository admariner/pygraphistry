...
* GFQL schema validation tracks columns removed by row projections, `group_by`, and `drop_cols`, while retaining extended projections and prefix grouping keys.
* GFQL: a `group_by` aggregation source naming an existing column whose name contains a dot or a minus (`analysis.cohorts.Activist`, `x.Pro-Vax`) was rejected as a missing column (E301 `analysis`, E301 `Vax`): schema validation parsed every source as a row expression and split it on the operator, so uploaded column names both executors would serve never reached them. A source that is literally a column of the active row table is now required as that column — the precedence pandas and polars already use (the pandas row pipeline checks the column before parsing, and the polars aggregate accepts only columns) — and a source that is not a column is still parsed as an expression.
...
* GFQL: the pandas and cuDF native op-list lanes (single-node lookup, seeded typed hop) record their resident-index engagement in `gfql_explain` (seams `native_seed_lookup` / `native_seeded_hop`), matching the polars lane; a lane that stops engaging is now visible instead of silently correct.
...
- **Bound-frame immutability is now the STATED contract, and every cache honors its recovery recipe**: frames handed to GFQL are treated as immutable (what makes verdict/index/fact reuse sound — the position of any engine with indexes); mutating a bound frame in place is undefined behavior, recovered by rebinding fresh frames or `gfql_clear_caches()`. The amplification round found the polars NaN clean-verdict cache violating the RECIPE, not just the contract: it was unregistered — invisible to `gfql_clear_caches()` and the cache completeness lock — so an in-place NaN injection had no supported recovery and silently changed WHERE rows. The cache is retained (O(1) repeat-ingest under the contract) and now registered/flushable, with contract-recipe pins. Also: the shortest-path cache's recyclable `id()` token became a strong-ref `is` guard, and a vacuous execution-spy test was rewritten against the real `chain_impl` seam with a positive control.
- **Known cross-engine divergences on degenerate inputs pinned as strict xfails** (`test_known_cross_engine_divergences.py`): #1808 (a dangling edge DESTINATION is matched by pandas/cuDF, dropped by polars — polars is the Cypher-correct side; the pandas endpoint gate is asymmetric) and #1739 (`HAS_<Label>` aggregation over duplicate node ids: the shared fast path keeps one row per id by frame order on pandas and both rows on polars). Both are pre-existing across releases, reproduce only on malformed input (dangling endpoints, duplicate ids), and are scheduled for the next cycle with parity-fixture re-baselining; the xfails make the fix PRs flip a pin instead of rediscovering the shapes.
- **#1824 groundwork + executable disclosure (polars-gpu fast paths still serve CPU work)**: every fast-path collect now goes through the target-honoring `lazy.collect`/`collect_all` (a source pin forbids bare collects), and the dispatch carries GPU-or-error NIE-decline plumbing — but the execution target deliberately stays CPU for now: flipping it on a real GPU exposed that the fast paths' EAGER arms (dense kernel, eager twins) never collect at all, and 86 polars-gpu tests were green only via the CPU mislabel. The full per-arm GPU-or-decline fix is #1824, next cycle; until then a strict xfail (`test_1824_polars_gpu_fast_path_serve_is_gpu_or_decline`) keeps the mislabel executable, and polars-gpu OLAP fast-path timings must be read as CPU-served.
- **LazyFrame input crashed undirected Cypher shapes on polars (#1740)**: the traversal joins the user's frames against eager wavefronts at many seams, and polars joins do not mix eagerness — lazy nodes (and for varlen, lazy edges) crashed with `TypeError: expected 'other' to be a 'LazyFrame'`. Patching one seam left varlen and multi-hop undirected crashing, so the polars chain entry now normalizes input eagerness once (LazyFrame is an accepted input format; the chain re-lazifies internally where plans benefit). Pinned: three undirected shapes × all four lazy/eager combinations, pandas parity, eager results.
- **Two-hop `count(*)` could return a STALE answer after an in-place frame mutation (#1825)**: the equal-domain degree counts were memoized by `setattr` onto the caller's `Plottable` keyed by `id()` — stable across in-place mutation and reusable after GC, the exact BLOCKER-1 pattern the same file forbids. The memo is removed rather than re-keyed: within one execution it never hit (one call per query), and CROSS-call reuse belongs to the fingerprint-validated index layer (`gfql_index_all`'s degree facts), which invalidates on mutation and is faster than the memo was. Pinned with a string-id fixture on purpose — dense-integer-id graphs are served by the dense kernel before the degree-count path, so the issue's own int-id repro reads as a false "fixed". Repeat-vs-one-shot parity is pinned as an identity. (Benchmark note, measured on the release board: NO cell moves — the fused lazy lane had already superseded the memo's value, so the unindexed q8 median is byte-identical with the memo gone [13.60 ms @100k] and the typed-index headline holds [3.55 ms]; the memo's only surviving effect was the stale-answer bug.)
- **A polars dtype could classify as a string in the pushdown planner**: `pd.api.types.is_numeric_dtype` returns a confident False for polars dtypes (no exception, so no fallback ran) and the string check accepted nearly everything, so polars `Float64` classified as a string. Classification is now polars-first and keyed on the dtype's type name rather than its repr — parameterized dtypes like `Enum(categories=[...])` render their arguments, so a repr whitelist silently missed them. Positive and negative pins across String/Categorical/Enum vs numeric vs neither. A consequence users see: `STARTS WITH`/`ENDS WITH`/`CONTAINS`/`=~` and bare float comparisons in comma-pattern WHERE clauses now answer natively on polars (they previously raised) with pandas-oracle parity pinned.
- **A denying policy could be silently bypassed on mixed-frame graphs under AUTO**: the transitional policy guard keyed on frame shapes while engine routing keys on `resolve_engine`, so a polars-edges + pandas-nodes graph routed to the polars path with the policy attached — where postload/postchain hooks do not fire yet. The guard now uses `resolve_engine` itself as its predicate, so every graph AUTO would route to polars is guarded; mixed-frame and edges-only deny pins added.
- **Degree facts now build on real graphs: the density requirement was mine, not the math's.** The builder refused any gapped id space, and real node spaces are gapped (the benchmark's own is), so degree facts built on every test fixture and on zero real graphs — answers stayed correct via the scan, making the dead path invisible until an engagement probe caught it. Density is not required for the arrays: ids absent from the span contribute zero to the dot, so degrees now build over each edge partition's own endpoint span (bounded by the existing span cap), and only the kernel's separate domain proof requires density — as it already did. The obsolete pin asserting the opposite is flipped, fixtures now use interleaved ids by default, and the gapped-space case pins both fact construction and exactness.
- **Timing magnitudes move out of GFQL product code into pyg-bench, and the semi-join no-dedup argument becomes a test.** Perf numbers in a public repo double as an optimization roadmap, so eight magnitude claims across five modules now state the SHAPE of each constraint (the dedup pass dominates at multi-million-key scale; the zero-row join compounds once per node step; streaming loses at interactive sizes) without the figures. Where a comment carried a claim NO test covered, the test was added rather than the prose kept: `_semi` skips a `.unique()` on its key frame, and that is now pinned both ways — duplicate keys select exactly what one copy selects (eager and lazy), a semi-join cannot fan out where an inner join demonstrably does, non-matching keys actually drop rows, and an empty key frame selects nothing while keeping the schema. No product behavior changes; the five non-test files are AST-identical, verified by `bin/ci_docs_only_check.py`.
- **cudf 26.02 removed `Series.dt.date` / `Series.dt.time`**, which the temporal comparison predicates called unguarded. They now fall back to equivalent formulations — a day-truncated datetime compares against a midnight `Timestamp` exactly as a date does, and a time-of-day timedelta against a `Timedelta` exactly as a time does — with the scalar paired to the dtype the truncation produces, so no comparison silently changes meaning. Pinned by simulating the accessor removal, so the regression is caught without a GPU.
